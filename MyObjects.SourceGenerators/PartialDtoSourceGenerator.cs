...
[Generator]
public class PartialDtoSourceGenerator : ISourceGenerator
{
    public void Initialize(GeneratorInitializationContext context)
    {
        ...
        context.RegisterForSyntaxNotifications(() => new PartialDtoClassReceiver());
    }

    public void Execute(GeneratorExecutionContext context)
    {
        var syntaxReceiver = context.SyntaxReceiver as PartialDtoClassReceiver;
    ...
    private void Generate(GeneratorExecutionContext context, ClassDeclarationSyntax dtoClassSyntax)
    {
        ...
        var entityTypes = dtoClassSymbol.AllInterfaces
            .Where(i => i.Name.StartsWith("IPartialDtoFor"))
        ...
        var implementation = entityTypes.SelectMany(t => 
            ...
            .Select(p => p.Type.NullableAnnotation == NullableAnnotation.Annotated ? $"public {p.Type} {p.Name} {{ get; set; }}" : $"public {p.Type}? {p.Name} {{ get; set; }}"));
...
{{
    ...
    public class PartialDtoClassReceiver : ISyntaxReceiver
    {
        ...
        public void OnVisitSyntaxNode(SyntaxNode syntaxNode)
        {
            if (syntaxNode is ClassDeclarationSyntax classDeclarationSyntax)
            {
                ...
                {
                    if (classDeclarationSyntax.BaseList?.Types.Any(t => t.Type is GenericNameSyntax gs && gs.Identifier.Text == "IPartialDtoFor") == true)
                    {
